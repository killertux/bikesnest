...
|---|---|---|---|
| Conta | endereço de e-mail; senha (armazenada apenas como *hash* criptográfico); nome de exibição (opcional); estado da conta; datas de criação e de verificação do e-mail | você | e-mail e senha: sim |
| Contribuições | locais de estacionamento, propostas de alteração, revisões, avaliações, verificações e fotos que você envia, com data e hora | você | não |
| Fotos | imagem reenviada em versões redimensionadas; **os metadados EXIF (incluindo localização GPS) são removidos** antes do armazenamento e o arquivo original não é publicado | você | não |
| Atividade privada | favoritos; registros de "estacionei aqui" | você | não |
| Denúncias | conteúdo denunciado, motivo e resultado da moderação | você / moderadores | não |
...
| Finalidade | Dados | Base legal (LGPD, art. 7º) | Base legal (GDPR, art. 6º) |
|---|---|---|---|
| Criar e manter sua conta; autenticar; verificar e-mail; redefinir senha; enviar e-mails transacionais | conta, dados técnicos | V – execução de contrato | 6(1)(b) – execução de contrato |
...
Adotamos medidas técnicas e administrativas proporcionais ao risco: conexões criptografadas (TLS), senhas armazenadas com algoritmo de *hash* resistente (Argon2), controle de acesso por papéis com registro de auditoria, remoção de metadados de fotos, limites de uso e proteção contra automações abusivas, cópias de segurança e um procedimento de resposta a incidentes. Em caso de incidente de segurança que possa acarretar risco ou dano relevante a você, comunicaremos a ANPD e os titulares afetados conforme a lei.

## 9. Seus direitos

Você pode, a qualquer momento (art. 18 da LGPD; arts. 15 a 22 do GDPR):
...
Esta Política é versionada. A data de vigência e o número da versão aparecem no topo da página, e as versões anteriores ficam disponíveis em [/privacy/versions](/privacy/versions). Se fizermos alterações relevantes, avisaremos por e-mail ou por aviso no serviço antes de entrarem em vigor.

## 13. Contato

Dúvidas, solicitações e reclamações sobre privacidade: **{{CONTACT_EMAIL}}**.
